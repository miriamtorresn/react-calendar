...
const getDay = (year: number, month: number, day: number, today: IDate): IMonthDay  => {
    ...
    return {
        ...
        isToday: date.getDate() === today.day,
...
export const getFullDate = (monthDay: IMonthDay, monthType: string = 'large') => {
    const month = getMonth(monthDay.date.month);
    return `${monthDay.date.day}/${monthType === 'large' ? month.large : month.short}/${monthDay.date.year}`;
};